...
\begin{intermission}
    In this section we formalize the \vocab{Markov Property} in continuous time.
    In simple words, this property states that the future of a process 
    depends only on its present state, not on its past history. 

    We start by introducing the \vocab{Markov Transition Function} which
    governs how a process moves between states. Via the 
    \vocab{Chapman-Kolmogorov equation} we ensure that moving from $s$ 
    to $t$ is the same as stopping at $u \in (s,t)$ and restarting.
    We then extend this notion via the \vocab{Strong Markov Property} 
    which says that the "memoryless" property holds even if the time 
    we stop to check the process is random, as long as it's a stopping time. 

    This set-up is then used to introduce some very useful results
    and concepts for our modelling endeavors:

    \begin{enumerate}
        \item The solutions of SDEs are Markov processes. Hence, we
    can use the current state of a system to forecast future 
    distributions without needing the entire historical data-set.
    \item By transitioning from the Markov process to the 
    \vocab{Infinitesimal Generator} and the \vocab{Semigroup}
    we can turn a stochastic path problem into a deterministic 
    PDE, which we can solve via finite-difference methods. 
    This saves us computational time, and resources, compared to  
    using Monte Carlo methods.
    \item \vocab{Càdlàg} functions are essential for modelling 
    "shocks" or jumps, such as sudden market crashes.
    \end{enumerate}
...
\begin{remark}
    In general $(\mathcal{F}_t)_{t \ge 0}$ is a filtration that contains $(\sigma(X_s, s \le t))_{t \ge 0}$ but without loss of generality we could just consider $\mathcal{F}_t = \sigma(X_s, s \le t)$.
\end{remark}
...
\begin{tikzpicture}[scale=1.2]
    % Custom colors matching the hand-drawn markers
    \definecolor{axisgreen}{RGB}{125, 195, 75}
    \definecolor{curvegreen}{RGB}{100, 180, 60}
    \definecolor{textorange}{RGB}{245, 120, 0}
    \definecolor{markred}{RGB}{210, 20, 20}
    \definecolor{dashblue}{RGB}{30, 130, 240}

    % Redefined base curve to explicitly pass through (0,0)
    \def\basecurve{plot[smooth, tension=0.7] coordinates {(-0.3, 0.6) (0, 0) (1.0, -1.3) (2.4, -0.1) (4.2, 2.2)}}

    % --- First Graph (Left) ---
    \begin{scope}
        % Axes
        \draw[->, thick, axisgreen] (-0.5, 0) -- (4.5, 0) node[below right] {$t$};
        \draw[->, thick, axisgreen] (0, -1.8) -- (0, 2.5) node[left] {$\mathbb{R}^d$};

        % Curve
        \draw[thick, curvegreen] \basecurve;

        % Mark h (red dashed line)
        \draw[thick, markred, dashed] (0.9, 0) node[above=2pt] {$h$} -- (0.9, -1.25);

        % Label
        \node[textorange, right] at (2.8, 1.2) {$t \mapsto \omega(t)$};
    \end{scope}
    ...
    \begin{scope}[xshift=6.5cm]
        ...
        % Shifted Curve (Solid Green)
        \begin{scope}
            \clip (-0.5, -1.8) rectangle (4.5, 2.8);
            % Shifted left by the exact value of h (0.9)
            \draw[thick, curvegreen, xshift=-0.9cm] \basecurve;
        \end{scope}

        % Original Curve Reference (Dashed Blue)
        \draw[thick, dashblue, dashed] \basecurve;

        % Mark h (blue tick)
        \draw[thick, dashblue] (0.9, 0.1) node[above=2pt] {$h$} -- (0.9, -0.1);

        % Label
        \node[textorange, anchor=south west] at (2.0, 2.0) {$t \mapsto \theta_h \omega(t) = \omega(h+t)$};
    \end{scope}
\end{tikzpicture}
...
\begin{homework}
    Prove the above result using the same tricks as before.
\end{homework}
